...
def record_cpu_in_region(region, ymd):
    ...
    for key_name, cpus_by_resource_id in data_by_name.items():
        ...
        S3.Object(S3_BUCKET, f"{LAMBDA_KEY_PREFIX}cpu-utilization/{region}/{ymd['ymd_str'][:4]}/{ymd['ymd_str'][4:6]}/{ymd['ymd_str'][6:8]}/{ymd['ymd_str']}_{key_name_for_s3}.json").put(Body = json.dumps(cpus_by_resource_id, ensure_ascii=False))
        time.sleep(0.02)
