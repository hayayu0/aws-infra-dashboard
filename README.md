...
- CPU使用率を重ねて表示  OFFへ変更可能
- CloudFront + S3 + Lambda によるサーバーレス構成
- Excelファイル形式(xlsx)でエクスポート
- Nameタグの右のアイコンを押してインスタンスごとの履歴画面へ遷移
- 多彩な絞り込み表示
  - ドロップダウンの選択肢での絞り込み
  - URLパラメーター(クエリ文字列)での絞り込み
  - Nameタグの値を正規表現によって絞り込み

## カスタム機能

デプロイ時(★) または デプロイ後の `config.js` の編集(★★) をすることで、通常以上のカスタム機能が利用できます。

- マルチアカウントに対応。複数アカウントを同一テーブルで表示（設定方法：★★）（後述）
- マルチリージョンに対応。一時点では単一リージョンの表示で、切り替えて表示（設定方法：★ または ★★）
- アクセス元IPアドレスの範囲指定による接続制限（設定方法：★ または WAFのルールを編集）
...
- PCでの利用を前提としています。スマートフォン向けのレイアウト調整は行っていません。
